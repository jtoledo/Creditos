class CaprapchequeController < ApplicationController
  include AjaxScaffold::Controller
 
  def index
  		
		#cargar las opciontes
		@creditos=VrCcCredito.find_by_sql("select * from vr_cc_creditos where not cancelado and not entregado_boo order by cliente")
		@opcredito=[[]]
		@opcredito[0]="<SELECCIONE UN CLIENTE>"
		count=1
		for el in @creditos
			@opcredito[count]=el.id.to_s+"- "+el.cliente.to_s,el.id.to_s
			if count==1 
				@credito=el.id
		...
		if request.post?
			if defined?(params[:id][:id])
				@parametro=params[:id][:id]
			else
				@parametro=params[:cliente][:idc]
			end
		else
			@parametro=@credito
		end
